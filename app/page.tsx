      ...
      <main className="min-h-screen bg-[#030303] text-white">
          ...
          <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-4 sm:px-6 min-[700px]:max-w-[900px] min-[700px]:px-6 min-[700px]:py-3 lg:px-7">
            <Link href="/" className="flex min-w-0 items-center gap-3" aria-label="ROMIL PLUS">
              <div className="h-[78px] w-[92px] shrink-0 overflow-visible sm:h-[92px] sm:w-[110px] min-[700px]:h-[82px] min-[700px]:w-[122px] lg:h-[88px] lg:w-[132px] xl:h-[92px] xl:w-[138px]">
                <img src="/logo-romil-plus.png" alt="ROMIL PLUS" className="h-full w-full object-contain min-[700px]:scale-[1.08] lg:scale-[1.1]" />
              </div>
            </Link>
        ...
        <section className="mx-auto max-w-5xl px-4 pt-5 sm:px-6 min-[700px]:max-w-[900px] min-[700px]:pt-4">
          <div className="rounded-3xl border border-[#a97a25]/45 bg-[radial-gradient(circle_at_90%_100%,rgba(230,184,79,.16),transparent_33%),linear-gradient(135deg,#0d0d0d,#111)] px-5 py-7 text-center shadow-[0_0_28px_rgba(197,139,33,.08)] sm:py-9 min-[700px]:py-5">
            <h1 className="text-3xl font-black sm:text-4xl min-[700px]:text-[28px]">Bienvenido a <span className="text-[#e3b64f]">ROMIL PLUS</span></h1>
            <p className="mt-2 text-sm text-white/65 sm:text-base min-[700px]:text-sm">Tu tienda de suscripciones digitales premium</p>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-5xl px-4 py-6 sm:px-6 min-[700px]:max-w-[900px] min-[700px]:py-4">
          <div className="rounded-3xl border border-white/10 bg-[#080808] p-4 sm:p-6 min-[700px]:p-4">
            ...
            <div className="mt-6 space-y-3 min-[700px]:mt-4 min-[700px]:space-y-2.5">
              {featured.map((p)=><Link key={p.id} href={`/producto/${p.id}`} className="grid grid-cols-[112px_1fr_auto] items-center gap-4 rounded-2xl border border-white/10 bg-gradient-to-r from-[#101010] to-[#0a0a0a] p-3 transition hover:border-[#e3b64f]/40 sm:grid-cols-[140px_1fr_auto] min-[700px]:grid-cols-[104px_1fr_auto] min-[700px]:gap-3 min-[700px]:p-2.5">
                <img src={p.image || "/logo-romil-plus.png"} alt={p.name} className="h-28 w-28 rounded-2xl bg-white object-cover sm:h-32 sm:w-32 min-[700px]:h-24 min-[700px]:w-24"/>
                <div className="min-w-0"><p className="text-lg font-black sm:text-xl min-[700px]:text-base">{p.name}</p><p className="mt-2 text-sm text-white/60 min-[700px]:mt-1 min-[700px]:text-xs">Suscripción: <span className="font-bold text-[#e3b64f]">{p.duration}</span></p><p className="mt-3 inline-block rounded-xl border border-[#b17f26]/40 px-3 py-2 text-lg font-black text-[#e3b64f] min-[700px]:mt-2 min-[700px]:px-2.5 min-[700px]:py-1.5 min-[700px]:text-base">S/ {Number(p.price).toFixed(2)}</p></div>
                <span className="text-3xl text-white/75 min-[700px]:text-2xl">›</span>
              </Link>)}
            </div>
            <Link href="/catalogo" className="mt-5 flex min-h-[58px] items-center justify-center gap-2 rounded-2xl border border-[#d5a536]/55 bg-black px-4 font-black min-[700px]:mt-4 min-[700px]:min-h-[48px] min-[700px]:text-sm">VER TODOS LOS PRODUCTOS <ShoppingBag size={18} className="text-[#e3b64f]"/></Link>
          </div>
          ...
          <Link href="/catalogo" className="mt-4 flex items-center gap-4 rounded-3xl border border-[#a97a25]/45 bg-[linear-gradient(90deg,#0a0a0a,#171107,#0b0b0b)] p-4 min-[700px]:mt-3 min-[700px]:p-3">
            ...
            <div className="min-w-0 flex-1"><p className="font-black text-[#e3b64f] min-[700px]:text-sm">PROMOCIÓN POR TIEMPO LIMITADO</p><p className="mt-1 text-sm text-white/60 min-[700px]:text-xs">Descuentos especiales en nuestras suscripciones</p></div>
