...
export function ProductCard({ product }: { product: Product }) {
  ...
  const imagePanelSize = isSupportProduct ? "min-[900px]:!h-auto min-[900px]:!w-full min-[900px]:aspect-square" : "";
  ...
  return (
    <article className={`group mx-auto flex h-[540px] w-full max-w-[310px] flex-col rounded-[18px] border-2 border-[#d6a83f] p-3 transition duration-200 min-[760px]:h-[410px] min-[760px]:max-w-none min-[760px]:rounded-2xl min-[760px]:border min-[760px]:p-3 xl:h-[430px] ${justAdded ? "bg-black shadow-[0_0_0_1px_rgba(255,224,140,.35),0_0_22px_rgba(214,168,63,.24),0_22px_50px_rgba(0,0,0,.72)] scale-[0.99]" : "bg-[linear-gradient(180deg,#111,#0b0b0b)] shadow-[0_0_0_1px_rgba(255,224,140,.12),0_18px_44px_rgba(0,0,0,.28)] hover:-translate-y-1 hover:border-[#efc75e]"}`}>
      ...
      {isSupportProduct ? <div className="mt-auto border-t border-[#d6a83f]/45 pt-3 min-[760px]:pt-1.5 min-[900px]:pt-2"><p className="text-[13px] font-bold leading-5 text-white min-[760px]:text-[7px] min-[760px]:leading-[10px] min-[900px]:text-[8px] min-[900px]:leading-[11px]"><span className="text-[#f6bf2f]">Precio:</span> ESCRIBIR A SOPORTE Y PREGUNTAR</p><p className="mt-1 text-[12px] leading-5 text-white/55 min-[760px]:mt-0.5 min-[760px]:text-[6px] min-[760px]:leading-[9px] min-[900px]:text-[7px] min-[900px]:leading-[10px]">YA QUE LOS PRECIOS VARÍAN DE ACUERDO A LO REQUERIDO Y LA CANTIDAD.</p><Link href="/soporte" className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-[#f6bf2f] px-4 py-3 text-[15px] font-black text-black transition hover:bg-[#ffd052] min-[760px]:mt-1.5 min-[760px]:rounded-md min-[760px]:py-1.5 min-[760px]:text-[8px] min-[900px]:mt-2 min-[900px]:py-2 min-[900px]:text-[9px]"><MessageCircleMore size={14}/> Consultar en soporte</Link></div> : <><div className="mt-auto flex items-end justify-between gap-2 pt-2 min-[760px]:pt-2"><div className="min-w-0"><span className="text-[25px] font-black min-[760px]:text-[20px] xl:text-[22px]">S/ {displayedPrice.toFixed(2)}</span>{isGemini ? <><p className="mt-0.5 text-[12px] text-white/45 min-[760px]:text-[8px]">{geminiQuantity} {geminiQuantity === 1 ? "cuenta" : "cuentas"} · S/ {geminiUnitPrice.toFixed(2)} c/u</p><p className="mt-0.5 text-[12px] text-white/45 min-[760px]:text-[8px]">Suscripción: {product.duration}</p></> : <p className="mt-0.5 text-[12px] text-white/45 min-[760px]:text-[8px]">{selectedVariant ? selectedVariant.label : `Suscripción: ${product.duration}`}</p>}{product.guarantee && <p className="mt-1 inline-flex items-center gap-1 rounded-lg border border-[#d6a83f]/35 px-2 py-1 text-[11px] font-semibold text-[#f0cd78] min-[760px]:text-[8px]"><ShieldCheck className="h-3 w-3"/> Garantía: {product.guarantee}</p>}</div><button type="button" onClick={addToCart} className={`shrink-0 rounded-xl px-3 py-2.5 text-[14px] font-bold transition duration-200 min-[760px]:rounded-lg min-[760px]:px-3 min-[760px]:py-2 min-[760px]:text-[10px] ${justAdded ? "bg-black text-white ring-2 ring-white/30" : "bg-[#f6bf2f] text-black hover:bg-[#ffd052]"}`}><ShoppingBag className="inline" size={14}/> {justAdded ? "Añadido" : "Añadir"}</button></div><Link href={`/producto/${product.id}`} className="mt-2 block text-center text-[14px] text-white/50 hover:text-white min-[760px]:mt-2 min-[760px]:text-[9px]">Ver detalles</Link></>}
    </article>
  );
}
