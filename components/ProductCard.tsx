...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className={`group flex h-full flex-col rounded-3xl border p-4 transition duration-200 sm:p-5 lg:p-6 ${justAdded ? "border-black bg-black shadow-2xl shadow-black/80 scale-[0.99]" : "border-white/10 bg-white/[0.045] hover:-translate-y-1 hover:border-[#d6a83f]/40 hover:bg-white/[0.065] lg:shadow-[0_18px_50px_rgba(0,0,0,.22)]"}`}>
      <div className={`mx-auto mb-5 flex w-full items-center justify-center overflow-hidden rounded-2xl lg:aspect-square lg:max-w-[360px] ${isCapCut || isYouTube ? "h-52 bg-white p-3 lg:h-auto" : "h-48 bg-gradient-to-br from-cyan-500/10 via-violet-500/15 to-fuchsia-500/10 lg:h-auto"}`}>
        {product.image ? <img src={product.image} alt={product.name} className="h-full w-full object-contain object-center"/> : <span className="text-4xl font-black text-white/85 lg:text-5xl">{product.name.split(" ").map((word) => word[0]).join("").slice(0, 2)}</span>}
      </div>
