...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className={`group flex h-full flex-col rounded-[22px] border p-3.5 transition duration-200 sm:p-4 min-[700px]:rounded-3xl min-[900px]:p-4 xl:p-5 ${justAdded ? "border-black bg-black shadow-2xl shadow-black/80 scale-[0.99]" : "border-white/10 bg-white/[0.045] hover:-translate-y-1 hover:border-[#d6a83f]/40 hover:bg-white/[0.065] min-[900px]:shadow-[0_18px_50px_rgba(0,0,0,.22)]"}`}>
      <div className="mx-auto mb-3.5 flex aspect-square w-full items-center justify-center overflow-hidden rounded-[18px] bg-white min-[700px]:mb-5 min-[900px]:rounded-2xl">
      ...
      <div className="mb-2.5 flex items-start justify-between gap-2 min-[700px]:mb-3 min-[700px]:gap-3">
        <div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-wider text-[#e2b44c] sm:text-[11px] min-[900px]:text-[11px] xl:text-sm">{product.category}</p><h3 className="mt-1 text-lg font-bold sm:text-xl min-[900px]:text-lg xl:text-2xl">{product.name}</h3></div>
      ...
      <div className="mt-auto flex items-end justify-between gap-2 pt-4 min-[700px]:gap-3 min-[700px]:pt-6"><div className="min-w-0"><span className="text-[22px] font-black sm:text-2xl min-[900px]:text-xl xl:text-3xl">S/ {displayedPrice.toFixed(2)}</span><p className="mt-1 text-[11px] text-white/45 sm:text-xs min-[900px]:text-[10px] xl:text-sm">{selectedVariant ? selectedVariant.label : `Suscripción: ${product.duration}`}</p>{product.guarantee && <p className="mt-1 text-[11px] font-semibold text-[#f0cd78] sm:text-xs min-[900px]:text-[10px] xl:text-sm">Garantía: {product.guarantee}</p>}</div><button type="button" onClick={addToCart} className={`shrink-0 rounded-xl px-3 py-2 text-[13px] font-bold transition duration-200 sm:text-sm min-[900px]:px-3 min-[900px]:py-2 min-[900px]:text-xs xl:px-5 xl:py-3 xl:text-base ${justAdded ? "bg-black text-white ring-2 ring-white/30" : "bg-white text-slate-950 hover:bg-[#f5df9b]"}`}><ShoppingBag className="inline" size={16}/> {justAdded ? "Añadido" : "Añadir"}</button></div>
